		...
		if (isPlaying && textContainerRef.current && scrollAreaRef.current) {
			...
			if (currentWord) {
				...
				const highlightedElement =
					textContainerRef.current.querySelector(
	...
	const handleFileUpload = useCallback(
		async (event: React.ChangeEvent<HTMLInputElement>) => {
			...
			try {
				...
				if (
					!Array.isArray(wordTimingData) ||
					wordTimingData.length === 0
				) {
				...
				setIsUploading(false);
				alert(
					`Error processing file: ${error instanceof Error ? error.message : "Please try again."}`,
				);
			}
		},
		[],
			...
			<div className="max-w-4xl mx-auto">
				...
				<div className="text-center mb-8">
					...
					<p className="text-gray-600">
						Upload a text file to read your story with guided
						highlighting
					</p>
				</div>
				...
				<Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
					<DialogContent className="sm:max-w-md">
						<DialogHeader>
							...
							<DialogDescription>
								Choose a .txt file containing your story to
								display it with timed reading highlights.
							</DialogDescription>
						</DialogHeader>
						...
						<div className="space-y-4">
							...
							<div className="text-xs text-gray-500 bg-gray-50 p-3 rounded-md">
								<strong>Tip:</strong> The story will be
								displayed with precise timing-based highlighting
								from your API.
							</div>
						</div>
					</DialogContent>
